...
public class SimplePALcodec {
        ...
        for (int y0 = 0; y0 < HEIGHT; y0 += 2) {
            ...
            for (int x0 = 0; x0 < WIDTH; x0++) {
                ...
                if (!bw) {
                    ...
                    chrominance[index] = 0f; // reset in bw
                }
    ...
    public static void decodeYC(final byte data[], final boolean bw) {
        ...
        for (int y0 = 0; y0 < HEIGHT; y0 += 2) {
            ...
            float oldLuminance = 1f; // reset luminance
            ...
            for (int x0 = 0; x0 < WIDTH; x0++) {
                ...
                if (p <= len - 3) { 
                    ...
                    if (bw) {
                        ...
                        final int rInt = Gfx.saturate(Math.round(gamma(saturate(r), 1.2f) * 255));
                        final int gInt = Gfx.saturate(Math.round(gamma(saturate(g), 1.2f) * 255));
                        final int bInt = Gfx.saturate(Math.round(gamma(saturate(b), 1.2f) * 255));

                        Color.RGBtoHSB(rInt, gInt, bInt, hsv);

                        hsv[1] = saturate(1.8f * hsv[1]);
                        final int color = Color.HSBtoRGB(hsv[0], hsv[1], hsv[2]);

                        data[p] = (byte) (color & 0xff);
