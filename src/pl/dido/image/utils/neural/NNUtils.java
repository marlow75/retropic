...
public class NNUtils {
		...
		try {
			...
			for (int i = 0; i < 256; i++) { // first 256 characters
				...
				for (int j = 0; j < 16; j += 2) {
					...
					input[j * 8 +  8] = (byteRead & 128) >> 7;
					input[j * 8 +  9] = (byteRead & 64) >> 6;
					input[j * 8 + 10] = (byteRead & 32) >> 5;
					input[j * 8 + 11] = (byteRead & 16) >> 4;
					input[j * 8 + 12] = (byteRead & 8) >> 3;
					input[j * 8 + 13] = (byteRead & 4) >> 2;
					input[j * 8 + 14] = (byteRead & 2) >> 1;
					input[j * 8 + 15] = (byteRead & 1);
				}
	...
	public static Vector<Dataset> loadData8x8(final InputStream inputStream) {
		int byteRead = -1;
	...
	public static Vector<Dataset> loadData4x8(final InputStream inputStream) {
		int byteRead = -1;
		...
		try {
			...
			for (int i = 0; i < 256; i++) { // first 256 characters
				final float[] answer = new float[256];
				final float[] input = new float[4 * 8];
				...
				for (int j = 0; j < 8; j++) {
					...
					input[j * 4 + 0] = (byteRead & 192) >> 6;
					input[j * 4 + 1] = (byteRead & 48) >> 4;
					input[j * 4 + 2] = (byteRead & 12) >> 2;
					input[j * 4 + 3] = (byteRead & 3);
				}
	...
	public static Vector<Vector<Dataset>> loadBatchData8x8(final InputStream inputStream, final int batchSize) {
		int byteRead = -1;
