...
public class QuickBitVector extends Object {
	...
	/**
	 * Returns a bit mask with bits in the specified range set to 1, all the rest
	 * set to 0. In other words, returns a bit mask having 0,1,2,3,...,64 bits set.
	 * If <tt>to-from+1==0</tt> then returns zero (<tt>0L</tt>). Precondition (not
	 * checked): <tt>to-from+1 &gt;= 0 && to-from+1 &lt;= 64</tt>.
	 *
	 * @param from index of start bit (inclusive)
	 * @param to   index of end bit (inclusive).
	 * @return the bit mask having all bits between <tt>from</tt> and <tt>to</tt>
	 *         set to 1.
	 */
	...
	/**
	 * Changes the bit with index <tt>bitIndex</tt> in the bitvector <tt>bits</tt>
	 * to the "clear" (<tt>false</tt>) state.
	 *
	 ...
	 * value are set to 0. If <tt>from &gt; to</tt> then returns zero (<tt>0L</tt>).
	...
	public static long getLongFromTo(long[] bits, int from, int to) {
		...
		long mask;
		if (fromIndex == toIndex) { // range does not cross unit boundaries; value to retrieve is contained in one
									// single long value.
		...
		// range crosses unit boundaries; value to retrieve is spread over two long
		// values.
	...
	/**
	 * Returns the index of the least significant bit in state "true". Returns 32 if
	 * no bit is in state "true". Examples:
	 * 
	 * <pre>
	0x80000000 --> 31
	0x7fffffff --> 0
	...
	/**
	 * Constructs a low level bitvector that holds <tt>size</tt> elements, with each
	 * element taking <tt>bitsPerElement</tt> bits.
	 *
	...
	/**
	 * Returns the index of the most significant bit in state "true". Returns -1 if
	 * no bit is in state "true". Examples:
	 * 
	 * <pre>
	0x80000000 --> 31
	...
	/**
	 * Initializes a table with numbers having 1,2,3,...,64 bits set. pows[i] has
	 * bits [0..i-1] set. pows[64] == -1L == ~0L == has all 64 bits set --> correct.
	...
	private static long[] precomputePows() {
		...
		// OLD STUFF
		/*
		 * for (int i=BITS_PER_UNIT+1; --i >= 0; ) { pows[i]=value; value = value >>> 1;
		 * System.out.println((i)+":"+pows[i]); }
		 */

		/*
		 * long[] pows=new long[BITS_PER_UNIT]; for (int i=0; i<BITS_PER_UNIT-1; i++) {
		 * pows[i]=Math.round(Math.pow(2.0,i+1))-1; System.out.println((i)+":"+pows[i]);
		 * } pows[BITS_PER_UNIT-1] = ~0L;
		 * System.out.println((BITS_PER_UNIT-1)+":"+pows[BITS_PER_UNIT-1]); return pows;
		 */
	}

	/**
	 * Sets the bit with index <tt>bitIndex</tt> in the bitvector <tt>bits</tt> to
	 * the state specified by <tt>value</tt>.
	 *
	 * @param bits     the bitvector.
	 ...
	 * If <tt>from &gt; to</tt> then does nothing. Precondition (not checked):
	 * <tt>to-from+1 &lt;= 64</tt>.
	 *
	 * @param bits  the bitvector.
